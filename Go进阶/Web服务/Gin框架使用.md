...
- 在生成表单页面时，生成一个唯一的令牌并存储在服务器端（例如，存储在用户的会话中），同时将令牌嵌入到表单中（例如，作为一个隐藏字段）。当表单提交时，服务器端检查提交的令牌是否与存储的令牌一致。如果一致，处理表单数据并删除令牌；如果不一致，说明是重复提交，拒绝处理。

2、使用Post-Redirect-Get模式

3、前端使用AJAX异步提交表单数据

- 表单提交后，立即使用 JavaScript 禁用提交按钮，防止用户多次点击提交。
