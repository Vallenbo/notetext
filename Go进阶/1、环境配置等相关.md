...
作用：包含了 Go 语言的标准库、编译器、命令行工具等。当你在编写 Go 代码时，如果使用了标准库中的包，编译器会从`GOROOT`指定的路径中查找这些包的源代码。

`GOPATH` 是工作目录，用于存放你的 Go 项目代码和相关的依赖。

- `src`：存放你的 Go 项目源代码和第三方库的源代码。每个项目或库都应该放在一个独立的目录下，目录名通常与导入路径相对应。例如，如果你有一个项目名为`myproject`，其导入路径可能是`github.com/yourusername/myproject`，那么这个项目的源代码应该放在`$GOPATH/src/github.com/yourusername/myproject`目录下。
- `pkg`：存放编译后的包文件。当你编译一个包时，生成的对象文件会放在这个目录下。这个目录的结构与`src`类似，但是只包含包的名称和版本信息。
- `bin`：存放可执行文件。当你使用`go install`命令安装一个可执行的包时，生成的可执行文件会放在这个目录下。
...
```go
// PATH路径中添加，GOROOT路径和GOPATH路径
变量名：GOROOT=D:\Program Files\Go\bin
变量值：GOPATH=D:\Program Files\GOPATH		# GOPATH为新建目录
```
