...
GO中除了基本的整型、浮点型、布尔型、字符串外，还有array数组、slice切片、stuct结构体、func函数、map、通道（channel）等。编译器在访问变量时，会遵守”由远到近”原则，先访问本地再访问其他包。
...
1、**直接通过索引取出字节：** 这种方法只适用于ASCII编码的字符串。
...
```go
fmt.Println("按位与：", a&b) // 输出 12，即 0000 1100
```

`|`参与运算的两数各对应的二进位相或。（两位有一个为1就为1）

```go
fmt.Println("按位或：", a|b)  // 输出 61，即 0011 1101
```

`^`参与运算的两数各对应的二进位相异或，当两对应的二进位相异时，结果为1。（两位不一样则为1）

```go
fmt.Println("按位异或：", a^b) // 输出 49，即 0011 0001
```
...
举个例子，现在有一个数组`a := [8]int{0, 1, 2, 3, 4, 5, 6, 7}`，切片`s1 := a[:5]`，相应示意图如下。<img src="https://www.liwenzhou.com/images/Go/slice/slice_01.png" alt="slice_01" style="zoom:33%;" />

切片`s2 := a[3:6]`，相应示意图如下：<img src="https://www.liwenzhou.com/images/Go/slice/slice_02.png" alt="slice_02" style="zoom: 33%;" />
...
`make([]T, len, cap)`		`T`:切片的元素类型、`len`:切片中元素的数量、`cap`:切片的容量
...
`defer语句`：存储在栈内（后进先出），定义的顺序与实际执的行顺序是相反的，也就是最先声明的最后才执（类似栈）。
`defer的作用`：多用于函数结束之前释放资源(文件句柄、数据库连接、socket连接)
...
**类型与接口的关系**：
​	1、一个类型可以实现多个接口	2、多种类型也可以实现同一接口
...
在这个示例中，我们定义了一个Animal接口，并让Cat和Dog结构体分别实现了该接口的Say方法。在主函数中，我们创建一个Animal类型的变量，然后分别把Cat和Dog对象赋值给这个变量，最后调用Say方法输出不同的声音。
