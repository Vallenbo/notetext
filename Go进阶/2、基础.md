...
`switch语句`中case表达式可以是任意类型（如函数），case表达式可以是多个值

可以使用fallthrough强制执行后面的case代码，且fallthrough必须是case子句的最后一行
...
**鸭子类型**：鸭子类型（Duck Typing）是一种动态类型的编程方式，其核心思想是 **关注对象的行为而非类型 **。鸭子类型的基本定义可以概括为：“如果看起来像鸭子、叫声像鸭子，那么它就是鸭子。”

使用接口实现面向接口编程的示例。下面程序也实现了多态
