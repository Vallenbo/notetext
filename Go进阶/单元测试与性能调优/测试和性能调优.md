...
**单元测试**：是软件开发中的一种测试方法，用于验证代码中的最小可测试单元（通常是函数或方法）是否按照预期进行工作。它的目标是隔离和测试代码中的每个独立功能，以确保其在各种情况下都能正确运行。
...
基准测试的主要目的是建立一个性能的基准线，用于比较不同系统或配置之间的性能差异。它通常用于评估硬件、软件版本或配置变化对性能的影响。
...
func BenchmarkSplit(b *testing.B) {
	for i := 0; i < b.N; i++ { 		// 使用go提供的包，测试得到一条基准线
		Split("沙河有沙又有河", "沙")
	}
}
```
...
1. 示例函数提供了可以直接运行的示例代码，可以直接在`golang.org`的`godoc`文档服务器上使用`Go Playground`运行示例代码。下图为`strings.ToUpper`函数在Playground的示例函数效果。

<img src="./assets/example.png" alt="example" style="zoom: 33%;" />
