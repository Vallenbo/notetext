...
2. `GOPATH`：`GOPATH` 是指向 Go 工作区（workspace）的环境变量。Go 工作区是您在开发过程中创建的项目和导入包的根目录。`GOPATH` 包含三个重要的子目录：`src`、`pkg` 和 `bin`。
...
root@lnb:~/go/src/test# go mod init src/test #由于GOPATH在根目录go下，这里再指定路径初始化
```
