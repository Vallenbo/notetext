...
==GO中除了基本的整型、浮点型、布尔型、字符串外，还有数组、切片、结构体、函数、map、通道（channel）等。==
编译器在访问变量时，会遵守”由远到近”原则，先访问本地再访问其他包
...
| 方法             | 解释              | 方法                                | 解释           |
| ---------------- | ----------------- | ----------------------------------- | -------------- |
| len(str)         | 求长度            | strings.HasPrefix,strings.HasSuffix | 前缀/后缀判断  |
| +或fmt.Sprintf   | 拼接字符串        | strings.Index(),strings.LastIndex() | 子串出现的位置 |
| strings.Split    | 分割得到slice切片 | strings.Join(a[]string, sep string) | 拼接           |
| strings.contains | 判断是否包含      | 将其转换成[]rune或[]byte            | 修改字符串     |
...
例如 `sort.Ints(arri)`，其中变量 `arri` 就是需要被升序排序的数组或切片。为了检查某个数组是否已经被排序，可以通过函数 `IntsAreSorted(a []int) bool` 来检查，如果返回 `true` 则表示已经被排序。
...
`for k, v := range scoreMap {}`：遍历map。遍历map时的元素顺序与添加键值对的顺序无关。
...
`defer语句`：存储在栈内（后进先出），定义的顺序与实际执的行顺序是相反的，也就是最先声明的最后才执行。
