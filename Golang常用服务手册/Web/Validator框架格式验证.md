...
gin框架使用[github.com/go-playground/validator](https://github.com/go-playground/validator)进行参数校验，我们需要在定义结构体时使用 `binding` tag标识相关校验规则，可以查看[validator文档](https://godoc.org/github.com/go-playground/validator#hdr-Baked_In_Validators_and_Tags)查看支持的所有 tag。
...
下载使用：`go get github.com/go-playground/validator/v10`
...
var trans ut.Translator // 定义一个全局翻译器T

func InitTrans(locale string) (err error) { // InitTrans 初始化翻译器
	...
	if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
		...
		var ok bool
		trans, ok = uni.GetTranslator(locale) // 也可以使用 uni.FindTranslator(...) 传入多个locale进行查找
		if !ok {
...
func main() {
	...
	r.POST("/signup", func(c *gin.Context) {
		var u SignUpParam
		if err := c.ShouldBind(&u); err != nil {
			errs, ok := err.(validator.ValidationErrors) // 获取validator.ValidationErrors类型的errors
			if !ok {
				c.JSON(http.StatusOK, gin.H{ // 非validator.ValidationErrors类型错误直接返回
					"msg": err.Error(),
				})
				return
			}
			c.JSON(http.StatusOK, gin.H{ // validator.ValidationErrors类型错误则进行翻译
				"msg":errs.Translate(trans),
			})
			return
		}
		c.JSON(http.StatusOK, "success") // 保存入库等具体业务逻辑代码...
	})
...
```go
func customFunc(fl validator.FieldLevel) bool { // customFunc 自定义字段级别校验方法
...
```go
if err := v.RegisterValidation("checkDate", customFunc); err != nil { // 在校验器注册自定义的校验方法
	return err
}
```
...
func translate(trans ut.Translator, fe validator.FieldError) string { // translate 自定义字段的翻译方法
