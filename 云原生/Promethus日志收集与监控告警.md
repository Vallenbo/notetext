...
接下来就需要用到Prometheus的另一个组件：

**Alertmanager**，主要用来进行告警的通知，可以采用邮箱、钉钉、微信、Slack、Webhook等进行通知。
...
PromQL Web UI的Graph选项卡提供了简单的用于查询数据的入口，对于PromQL的编写和校验都可以在此位置。

1、我们可以先输入up，然后单击Execute，就能查到监控正常的Target，如图15.27所示。

![](./assets/15-27.png){: .zoom}

up命令可以查到所有正常监控的目标，如果监控的目标过多，可能会查询出来很多数据。

2、此时可以根据一些条件进行过滤，就像SQL语句的where语法。比如通过标签选择器过滤出job为node-exporter的监控。

语法为：up{job="node-exporter"}

3、注意此时up{job="node-exporter"}属于 **绝对匹配**，PromQL也支持**非绝对匹配**如下表达式：
...
3、假如想要查询Kubernetes集群中每个宿主机的磁盘总量，可以使用node_filesystem_size_bytes。

可以看到此时查询结果过多，我们可以进一步进行处理，比如只查询根分区的大小，根据前文提到的标签选择器，加上mountpoint匹配规则node_filesystem_size_bytes{mountpoint="/"}即可。
...
上述的PromQL查询出来的数据都是瞬时值，也就是Prometheus采集到的最新的数据，在时序数据库中被称为瞬时向量。因为

5、Prometheus本身是一个时序数据库，所以也可以查询某个指标一段时间内的数据，比如查询主机k8s-master01在最近5分钟可用的磁盘空间变化（见图15.32）：
...
上一小节通过PromQL语法查到了主机磁盘的空间数据。

<img src="./assets/15-33.png" style="zoom: 33%;" />

1、数据为16358440960，这个结果并不适合直接阅读，因为它返回的数据单位为字节（bytes，其他指标可能是其他单位）。可以通过以下命令将字节转换为GB或者MB：
...
2、不同的指标结合不同的表达式可以查询到不同的数据，比如要查询k8s-master01根区分磁盘的可用率，可以通过如下指令进行计算：
...
- node_filesystem_avail_bytes 为系统可用的磁盘空间，
- node_filesystem_size_bytes为磁盘空间的总大小

所以两者的比值即为磁盘空间的可用率。当然也可以去掉instance参数，查询所有主机根分区的可用率：
...
3、虽然可以通过上述语句查询到一些指标，但是这些指标可能并非我们最终想要的。
...
> 除了以上判断语法外，PromQL也支持如下判断：==：相等。!=：不相等。：大于。<：小于。=：大于等于。<=：小于等于。
>
...
> 除了and外，也支持or和unless：and：并且。or：或者。unless：排除。
>

4、由上可知，and是必须两个条件都符合，or只需要符合其中一个即可，读者可以自行练习。
...
上一小节使用PromQL查询了一些指标，接下来通过PromQL一些常用的函数对数据进一步处理。比如使用sum函数统计当前监控目标所有主机根分区剩余的空间：
...
```sh
# 由该条数据结合sum函数，可以根据statuscode字段统计请求数据：
sum(http_request_total) by (statuscode)
```

![](./assets/15-40.png){: .zoom}

可以看到状态码为404的请求次数是12，状态码为200的请求次数是7156。同时也可以根据statuscode和handler两个指标进一步统）：
...
```yaml
# increase用于计算在一段时间范围内数据的增长，rate和irate用于计算增长率。比如查询某个请求在1小时内增长了多少：
...
实际应用环境中如果需要了解更多关于PromQL函数的内容，读者可以从[官网查询](https://prometheus.io/docs/prometheus/2.4/querying/functions/)中继续学习。

## 8.Alertmanager告警入门

本节我们学习Prometheus的告警组件Alertmanager，如何使用一些常用的媒介发送告警通知。也会学习Alertmanager的路由规则，将告警进行分组。
...
# 监控etcd集群（云原生应用自带metrics接口）

监控流程

<img src="./assets/image-20240515214918648.png" alt="image-20240515214918648" style="zoom:33%;" />
...
```shell
$ curl --cert /etc/etcd/ssl/etcd.pem --key /etc/etcd/ssl/etcd-key.pem  https://192.168.1.201:2379/metrics -k 
# 这样也行
...
# [k8s中部署Prometheus监控全家桶](https://zhuanlan.zhihu.com/p/674028745)
...
![img](./assets/v2-453e1b0aa4e6a6766f95bdf40326e869_1440w.webp)
...
![img](./assets/v2-fada68ecd3d54acddb92d2e6e02dc74b_1440w.webp)
...
![img](./assets/v2-db81e4e8f04d8b5c08bad8085a865ec3_1440w.webp)
...
![img](./assets/v2-02a18129daee98dc1c2aa9e2b642eb29_1440w.webp)

### **Grafana验证**

选一台 node 节点ip+30300，即可访问grafana的 Web UI，默认用户名密码：admin/admin，登录会提示更改密码，登录以后，可以看到已经内置了不少监控大盘

![img](./assets/v2-87eb7c28044b7c35bea25e960c958719_1440w.webp)

集群资源监控

![img](./assets/v2-e04ff5f2e9e3ffa197088d70b14a28eb_1440w.webp)
